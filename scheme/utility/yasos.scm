...
(library (utility yasos)
  ...
  (define (method-table . methods)
    (let ((table (make-eq-hashtable (length methods))))
      (for-each (lambda (m)
                  (hashtable-set! table (car m) (cdr m)))
                methods)
      table))

  (define (look-up table name)
    ...
    (syntax-rules ()
      ...
      ((_ (<name> <inst> <arg> ...)) ; no body
       (define-operation (<name> <inst> <arg> ...)
         (error '<name> (format "Operation not handled: {:s}" <inst>))))
    ))
    ...
    (syntax-rules ()
      ((_ ((<name> <self> <arg> ...) <exp1> <exp2> ...) ...)
       (let ((table (method-table
                      ...
                      ...)))
         (make-instance (cut look-up table <>))))))
